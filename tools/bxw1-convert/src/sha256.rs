//! SHA-256, written here rather than depended on.
//!
//! The workspace replaces crates.io with a vendored source directory, so a new
//! third-party crate in a host tool would mean vendoring it and re-checksumming
//! the tree. BXW1 needs exactly one primitive (§0, "No new external crate") and
//! this is it. FIPS 180-4, streaming, no allocation beyond the 64-byte block.
...
impl Sha256 {
    ...
    pub fn finish(mut self) -> [u8; 32] {
        ...
        // `update` maintains `length`, which the padding has now disturbed; the
        // captured `bits` above is the value the standard requires.
    ...
    fn compress(&mut self, block: &[u8; 64]) {
        ...
        for index in 0..8 {
            self.state[index] = self.state[index].wrapping_add(h[index]);
        }
    }
}
