...
pub fn host_sign(message: &[u8]) -> [u8; 64] {
    SigningKey::from_bytes(&HOST_KEY_SEED).sign(message).to_bytes()
}
...
mod tests {
    use super::{
        build_host_key_blob, compute_exchange_hash, host_public_key, host_sign,
        write_mpint, x25519_public_key, x25519_shared_secret, HOST_KEY_ALGORITHM_NAME,
    };
    ...
    fn test_host_key_blob_format() {
        ...
        let key_len = u32::from_be_bytes([
            blob[key_len_off], blob[key_len_off + 1], blob[key_len_off + 2], blob[key_len_off + 3],
        ]) as usize;
    ...
    fn test_exchange_hash_is_deterministic() {
        ...
        let h1 = compute_exchange_hash(b"V_C", b"V_S", b"I_C", b"I_S", &blob[..blob_len], &q, &q, &k);
        let h2 = compute_exchange_hash(b"V_C", b"V_S", b"I_C", b"I_S", &blob[..blob_len], &q, &q, &k);
        assert_eq!(h1, h2);
    }
}
