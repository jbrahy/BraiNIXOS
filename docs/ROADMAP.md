...
- **AS-1** Boot stub: aarch64 target spec and linker script, entry assembly, EL2→EL1, MMU init, exception vectors, generic timer, SVC entry, RNDR/PAC-BTI — **the absorbed Phase 4 core (#22)** — plus Image4/kmutil delivery, entry state established immediately (inherit nothing), boot-args + ADT consumption, s5l UART console, watchdog reset. **S** impl / **O** review. **Deps: AS-0 alone** (#22). **Exit criterion: BraiNIX prints its invariant banner over serial on the M2 Pro mini.** *This is now a materially larger task than it was, and the first place any of it runs is hardware.*
