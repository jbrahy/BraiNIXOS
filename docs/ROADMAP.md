...
|---|---|---|---|
| C1 | **AS-1b** — EL2→EL1, MMU init, exception vectors, generic timer, SVC entry, RNDR/PAC-BTI, watchdog | **STARTED 2026-08-14**: `src/aarch64-mmu/` encodes VMSAv8-64 descriptors in which W^X is **unrepresentable** — `Permission` has three variants and no fourth — with three Kani proofs over every address, permission, level and granule. The obligation the HAL's cancellation moved here is discharged for the encoding. | ~~The table walk, TTBR programming~~ — **both done and hardware-verified 2026-08-16.** The walk agrees with the MMU's own `AT s1e2r` on the live tables (16 KiB granule, 4 levels, terminating on a block at level 2). `TTBR0_EL2` was programmed with a root table this image placed and restored cleanly. Still open: whether *our own* built tables do what the manual says, which needs the table builder that does not exist yet. |
