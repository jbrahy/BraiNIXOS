...
| 33 | **Dev-dependencies are unrestricted.** The zero-external-crate rule applies to **what ships**, not to benchmark harnesses, property-test drivers or fuzzers. A dev-dependency never runs on the machine, never enters an Image4 payload, and cannot affect a reproducible build of one, so the dependency-closure principle was never about it. Made explicit because #28 makes every trade conditional on a *measured* win: measurement tooling is now load-bearing, and bad measurement is a correctness problem rather than a convenience one. The line that holds: a dev-dependency may never drift into shipped code. | **2026-08-17** |
...
- **AS-1** **SLICED (2026-08-10).** AS-1 as written below is a dozen independent subsystems, and one spec across all of them would be written entirely against unverified assumptions. It is now delivered in slices, each with its own design doc. **AS-1a is COMPLETE to the hardware gate (`e90ea1e`)**; the rest are unstarted.
  ...
  - **AS-1b — the aarch64 core. COMPLETE, 2026-08-17. Every item verified on the machine.**
    ...
    | item | state |
    | --- | --- |
    | Exception vectors | **done** — a `brk` at EL2 lands on vector index 4 (current EL, `SP_ELx`, synchronous), `ESR 0xf2000000` `EC 0x3c`, handler resumes past it, `VBAR_EL2` restored |
    ...
    | Own tables **installed** | **done, 2026-08-16.** `TTBR0_EL2` pointed at a root this repository *built*, not a copy of the machine's: an identity map of all 32 GiB of DRAM, the running code fetched through it, then restored. **All of DRAM costs three tables** — at a 16 KiB granule one level-2 table spans 64 GiB in 32 MiB blocks — which is what makes mapping everything the *safe* choice rather than the expensive one. `p.call` runs on **m1n1's** stack, and `boot_args`, m1n1's code and the payload are scattered across its heap; a mapping sized to what we think we need is a guess whose failure mode is a hang. Two things kept it from being a coin flip: the **attributes are lifted from a live block descriptor** (`0x1000c000601` → `0x600`, `AF=1 SH=2 AP=0 AttrIndx=0`) rather than invented, because memory type is an index into `MAIR` and a plausible constant yields a mapping that resolves and cannot be executed; and **every address the window touches is walked twice** — once by `aarch64_walk` over the new table, once by the MMU via `AT s1e2r` — with a single disagreement aborting the switch before `TTBR0_EL2` is written. Five checked, zero mismatches |
    | EL2→EL1 | **done under m1n1, 2026-08-16, and the earlier "belongs in the boot path" verdict was wrong.** The excursion clears `TGE`, programmes EL1's regime through the `_EL12` aliases, `eret`s to EL1, reads `CurrentEL` there (**EL1**), returns via `HVC` on vector 8, and restores `HCR_EL2` byte for byte. Two defects had made it look impossible, and neither was the reconfiguration the old note blamed. **The vector tail installed `ELR_EL2` and never `SPSR_EL2`**, so the return from EL1 resumed at the landing pad *at EL1*, where restoring `HCR_EL2` is undefined — invisible to a `brk` at EL2, where the stale value happens to agree. And **the payload was being loaded at an unaligned address**: `VBAR` ignores bits [10:0], so a misaligned vector table is not rejected, exceptions simply branch short of it, and adding 2 KiB of table was enough to turn a working probe into a hang. EL1 now has its own `MAIR`, `TTBR1` and vector table, and `AT S1E1R` answers "can EL1 reach this" before anything risks the machine |
    | SVC entry | **done, 2026-08-16.** `svc #0x42` at EL1 dispatches to EL1's own vector table with `ESR_EL1 = 0x56000042` — `EC 0x15` (SVC from AArch64), `ISS 0x42`, the immediate — and **EL1 carries on from the instruction after it**. That second half is the one a trap test cannot show: reaching a handler proves dispatch, resuming proves the return path. The handler splits on `ESR_EL1.EC`, not on the vector index, because vector 4 is `SVC` *and* every data abort, alignment fault and undefined instruction EL1 can raise — returning from those would re-execute the faulting instruction forever. `ELR_EL1` already points past the `svc`, so it is **not** advanced the way the EL2 handler advances past a `brk`; doing that would skip a real instruction of the caller. EL0→EL1 landed separately, below |
    | **EL0** | **done, 2026-08-17. Userspace exists.** Code ran unprivileged, made a system call that **returned to EL0**, then a second that left through EL1 — `ESR_EL1 = 0x56000056`, `EC 0x15`, `ISS 0x56`, and `SPSR_EL1.M = 0` (**EL0t**), which is the field that distinguishes this from an `SVC` made at EL1 and without which the claim is unsupported. Two calls, because one proves half: a syscall that *returns* to userspace is what a kernel does constantly and no trap test shows it. The regime is all of DRAM as kernel-only blocks except the 32 MiB containing the user page, laid out as 16 KiB pages so exactly **one** carries `AP[1]`; that page also gets `PXN` (EL1 must not execute what EL0 can write) and everything else gets `UXN`. Marking the *block* accessible instead would have been one line shorter and would have handed userspace 32 MiB containing the kernel's own code. **The gate is three `AT` instructions, and the one that matters must fail**: `AT S1E0R` on the kernel stack comes back `PAR_EL1 = 0x81f`, `F=1`, `FST 0xf` — permission fault. Without that question a regime that made all of DRAM EL0-accessible would pass every other check written. The `eret` does not happen unless all three answer correctly, and `AT` cannot fault, so the gate is free |
    | RNDR / PAC-BTI | **detection done; PAC enabling done and proven; one gap, stated.** `ID_AA64ISAR0_EL1 = 0x0221100110212120`: the RNDR field is **zero**, so this part has **no hardware RNG** and the kernel needs another entropy source. `ID_AA64ISAR1_EL1 = 0x0010111110211402`: `APA` (QARMA) is 0 while `API` is **4** — Apple implements PAC with its own algorithm *and* `FEAT_FPAC`, so checking only the QARMA field concludes PAC is absent on hardware that has it. `ID_AA64PFR1_EL1 = 0x21`: BTI present. **Enabling: `SCTLR` `EnIA`/`EnIB`/`EnDA`/`EnDB`/`BT1` all set and all read back stuck** (`0x30901185` → `0x10f8903185`), and authentication is live rather than merely enabled: signing changes the pointer, authenticating reverses it, and a signature with one bit flipped is **rejected with a synchronous exception** — `FEAT_FPAC` faulting, exactly as `API = 4` says it should. **The key is now this kernel's**, derived from the boot seed — see the entropy row below |
    | Entropy source | **done, 2026-08-17, and it had to be measured rather than looked up.** This part has no `RNDR`, so the source is `/chosen/random-seed`: 64 bytes iBoot leaves in the device tree. That it *exists* proves nothing — a constant baked into the image would look identical — so it was read across four boots and came back `7558166c…`, `3a182c6d…`, `fdee3955…`, `4825d9b5…`, all different, 63-64 of 64 bytes non-zero. `/chosen/cl4-entropy` is the other candidate and is **192 bytes of zeros** on this machine; it is recorded here so nobody spends the same hour on it. The seed is **hashed with a domain separator, not sliced**: firmware bytes should not reach a key register as themselves, and PAC's five key pairs plus every later consumer would otherwise be correlated slices of one 64-byte buffer. It is **erased on use**, verified by reading it back after — 0 of 64 non-zero — because deriving from it and leaving it in DRAM keeps a key-equivalent readable for the rest of the boot, and an erase the optimiser deleted looks exactly like one that worked. `SeedQuality::usable` refuses an absent, short or never-written buffer, and refusing is the correct outcome: an all-zero key authenticates perfectly and looks like a mitigation |
    | BTI **enforcement** | **done, 2026-08-17.** `SCTLR.BT` on its own is inert — BTI constrains branches only into pages whose descriptors carry `GP`, so until this kernel owned its own tables there was nothing to set it on, and every test of "BTI enabled" passed while the feature could not fire. With a regime whose descriptor reads `0x401000cebc603` (`GP=1`) installed in `TTBR0_EL2`, a `blr` to a plain instruction in that page is **rejected**: `ESR_EL2 = 0x36000002`, `EC 0xd` (Branch Target Exception), `BTYPE 2` (indirect call). A `blr` to a `BTI c` in the *same page* does not fault, which is the half that proves it discriminates rather than rejecting every indirect branch — a mis-set `SCTLR` or the wrong landing-pad encoding would produce the first result without the second. **Only the page under test is Guarded**, deliberately: BTI is decided by the page the *target* is in, so guarding all of DRAM would apply it to the exception handler, and a Branch Target Exception raised inside the handler for a Branch Target Exception is an unrecoverable loop that would present as BTI not working. `BLR` needs `BTI c`, `BR` needs `BTI j`; landing a `BLR` on a `BTI j` faults and looks exactly like the feature being broken |
    | Watchdog reset | **done, and the machine reset to prove it.** ADT lookup of `/arm-io/wdt` yields `0x29e2c4000`, the same address m1n1 independently printed. Armed in a single proxy session: `COUNT` zeroed by our write, ran up at 24.2 M ticks/s, hit the alarm at 33.5 M, and the proxy died mid-poll. Doubles as the **signalling channel** for anything running after m1n1 is chainloaded away, which `BRINGUP_PLAN.md` named before there was a way to use it |
    | Secondary-CPU release | **done, 2026-08-17. Two CPUs are running.** cpu1 came out of reset into code this kernel wrote and reported `MPIDR_EL1 = 0x80000001` -- aff0 = 1, against the boot core's `0x80000000`. That comparison is the measurement: a magic word only proves *something* wrote the buffer, a different affinity proves it was a different core. It arrived at **EL2** with `SCTLR.M = 0`, exactly a core out of reset, 176 ticks (7.3 us) after release. **Apple does not do PSCI or spin-tables** -- the other nine cores are powered down, and starting one is a reset vector written into that core's own `cpu-impl-reg` window plus two MMIO writes at `pmgr + 0x28000` (`0x28e0a8000`). `RVBAR` is writable on this part and the write **clears the lock bit**; the address field is bits **43:12**, not m1n1's `GENMASK(47,12)`, because bits 47:44 carry die/cluster/core identity and read back unchanged -- a mask that includes them makes a successful write look rejected, which is exactly the wrong answer the first measurement gave. The release is refused unless the readback confirms the vector took, because a core started at an address we did not choose cannot be stopped. **The hard part is cache coherency, not architecture**: the secondary runs with its MMU off, so its accesses bypass the cache while the boot core's writes may still be dirty in it -- the stub and its report buffer are cleaned with `dc civac` (point of coherency, not `dc cvau`'s point of unification) before release, and the boot core invalidates before every poll. One shot per boot: the stub parks in `wfe` and nothing here can recall it |
    | Timer *interrupt delivery* | **done, and it needed no AIC.** Armed for 6000 ticks and the interrupt arrived on **vector 6** — FIQ from the current EL on `SP_ELx` — after 6004. The generic timer is wired straight to FIQ on this platform, so a vector table and a `DAIF` write are the whole requirement. The first attempt failed and corrupted its neighbours; the cause was ours, not the hardware's — the handler saved only `x29`/`x30` and clobbered `x0`-`x7`, which a synchronous trap tolerates and an asynchronous one does not |
    ...
    One command reproduces all of it: `bin/as-kernel-probe.sh`.

    The sequencing is the transferable part. Vectors came before the MMU because
    bringing up translation *is* faulting, and a fault with nowhere to land is
    indistinguishable from a hang. `TTBR` was then programmed with a **copy of the
    live root table** rather than a newly built one, so the register changed and the
    mapping did not: had it faulted, the fault was attributable to the switch alone.
    Debugging a new table builder and a `TTBR` switch simultaneously is how you get a
...
| A2 | **P3-T2** | `WEIGHTS_REGION` / `KV_REGION` reserved regions in `memory/`, **and the 4 KiB → 16 KiB page fix** | none | **DONE 2026-08-14** (layout only). `PAGE_SIZE_IN_BYTES` is architecture-selected, so **the kernel's host tests now run at 16 KiB** — 393 pass unchanged, which is the cross-check Phase 4's cancellation removed, recovered from the other direction. Regions are counted in pages; `kv_partition(index)` computes an address rather than looking one up, so partitions are disjoint by construction; non-overlap is a `const` assertion, so a colliding layout **fails to link**. Maps nothing, seals nothing, zeroizes nothing — those are A3's, the page table's, and the session's. |
| A3 | **P3-T3a** | `src/servers/modeld/` — one-shot loader hosting the finished `src/bxw1/` parser, exits before `inferd` starts | A2, P3-T3, P3-T5 | **STARTED 2026-08-14.** BXW1 §10.1's "the order is normative" is now a state machine that refuses every forward jump, and denial carries a duty — S0 denies with no copy, S1 onward denies *and zeroizes*, because from S1 the region holds attacker bytes. The manifest is three entries and the tests count them: `INV-MODEL-001`'s entire content is that number. 15 tests, 100% coverage, zero exemptions. Reads no storage and seals nothing; that half still needs the kernel. |
...
| A5 | **P2-T11** | `tools/bsp-client/` — host test client driving both session types and all six admin verbs | A1, A7 | **STARTED 2026-08-14.** The handshake encoders land, and with them the property that matters: the server's decoder now has a **peer** written from the same field tables, so its tests are two implementations agreeing rather than one reading of the spec checked twice. Admin verbs and the request encoders remain. |
| A6 | **P3-T9a** | Host-run datapath: `bsp-client` → `servd` → `inferd`, two isolated sessions, streamed tokens | A1, A4, A5 | **STARTED 2026-08-14** — `tools/datapath/`. Two clients complete **real** PSK handshakes, take separate slots and separate KV partitions, and cannot name each other's state; teardown leaves the survivor untouched; a forged `ClientHello` authenticates as nobody; admission refuses a ninth client that authenticated perfectly well. **Streamed tokens are absent, not stubbed** — that needs the transformer over real weights, which needs A3's chain. And there is no kernel here, so this is a property of the components, not of a booted system. |
| A7 | **P2-T14** | Admin verb dispatch — exactly six handlers, `CapAdmin` only | A1, P2-T5, P2-T7, P2-T12, A3 | **STARTED 2026-08-14.** The table and its refusals: six entries a test counts, no `rotate`, session type taken from the slot rather than the message, break-glass refused unconditionally, `load-weights` and `reboot` marked reboot-class. The handlers themselves need A3, A8 and A9. |
| A8 | **P2-T12** | Runtime key enrollment + HKDF ratchet in the credential store | A1, P2-T5 | **STARTED 2026-08-14.** The enrollment *policy*: break-glass untouchable on every transport, refused at the network listener, and checked on the **derived** handle so choosing the key material chooses nothing. The ratchet is `transport-crypto`'s and exists; persistence needs ANS2 (AS-4a). **Forward secrecy still does not hold until the store persists a ratcheted chain.** |
| A9 | **P2-T7** | `db/` reframed for the session table + serving log, with the cross-session non-interference proof | P2-T5 | **STARTED 2026-08-14.** The log takes the reader's own slot rather than a filter, so the cross-session read is not expressible, and a Kani proof states that over every pair of `usize`. Generic in capacity because the proof would not finish at 64 rows. Persistence in `db/` remains. |
...
| B2 | **X-T4** — in-tree SHA-256 and ChaCha20 | **SHA-256 ADOPTED IN TWO CRATES 2026-08-14 (`766176b`); ChaCha20 BLOCKED ON THE WRONG VARIANT.** `transport-crypto` (hmac, schedule) and `bxw1` (per-tensor and whole-blob digests) now hash with `brainix-sha256`, and `sha2` is gone from `transport-crypto` entirely — its FIPS 180-4 vector was checking the vendored crate and now checks ours. `bxw1` keeps `sha2` as a **deliberate** dev-dependency: fixtures built by one implementation and verified by the other make all 92 blob tests differential. `brainix-chacha20` now carries RFC 8439 §2.3.2 and §2.4.2 known-answer tests, both passing. **The blocker is a variant mismatch, not effort:** `record.rs` uses `ChaCha20Legacy` — 64-bit nonce, 64-bit counter, as `chacha20-poly1305@openssh.com` requires — and the in-tree crate implements RFC 8439's 96-bit-nonce, 32-bit-counter construction. One counter word at state[12] and three nonce words at [13], versus two and two. **Not interchangeable**, so the legacy variant must be written before the `chacha20` dependency can go. **Still owed after that:** the kernel's seven `sha2` consumers, and `src/sha256`'s own oracle. **The honest metric is external packages — 41 today, unmoved**, because a crate leaves the lock only when its last consumer does; total lock entries went *up*, since in-tree crates are packages too. *Historical, for the shape of the mistake:* | `src/sha256/` landed 2026-08-14 (`f5a3571`) and `src/chacha20/` is in the working tree, added to the workspace, **and does not currently compile** (`saturating_mul` on an inferred integer, `src/chacha20/src/lib.rs:93`). Meanwhile `src/transport-crypto/Cargo.toml` still names `sha2` and `chacha20` in both `[dependencies]` and `[dev-dependencies]`, so **the vendored crates are still what the serving transport executes** and `Cargo.lock` still carries 84 packages. The task is not writing the primitive; it is **deleting the dependency** — swap the imports, keep the vendored crate only as a test oracle in `[dev-dependencies]` against RFC 8439 and FIPS 180-4 vectors, then drop it from `vendor/` and watch the lock count fall. Until that swap lands, no document may describe the primitive set as in-tree, and X-T4's own verify criterion — *`Cargo.lock` crate count strictly decreases* — is unmet. |
...
|---|---|---|---|
| C1 | **AS-1b** — EL2→EL1, MMU init, exception vectors, generic timer, SVC entry, RNDR/PAC-BTI, watchdog | **STARTED 2026-08-14**: `src/aarch64-mmu/` encodes VMSAv8-64 descriptors in which W^X is **unrepresentable** — `Permission` has three variants and no fourth — with three Kani proofs over every address, permission, level and granule. The obligation the HAL's cancellation moved here is discharged for the encoding. | **Every named item is hardware-verified as of 2026-08-16, except one that the silicon settles.** EL2→EL1, MMU init (including tables *this repository built*, installed and run on), exception vectors, the generic timer and its interrupt, SVC entry, PAC/BTI enabling and the watchdog all have measurements in the AS-1 table above, reproducible with one command: `bin/as-kernel-probe.sh`. **RNDR is not open work — this part has no RNG.** `ID_AA64ISAR0_EL1 = 0x0221100110212120`, the `RNDR` field is zero, so there is nothing to enable. The entropy it would have supplied comes from `/chosen/random-seed` instead (see the entropy row), measured across four boots to be fresh, hashed rather than sliced, and erased on use — so PAC runs on a key **this kernel derived**, not firmware's. **EL0 and BTI enforcement both landed 2026-08-17** — userspace runs, makes a system call that returns to it, and cannot read kernel memory (proved by an `AT S1E0R` that must fail); and an indirect branch into a Guarded page is rejected unless it lands on a `BTI`. **Secondary-CPU release landed 2026-08-17 as well: two CPUs are running.** Every item named in this row is now hardware-verified, reproducible with one command: `bin/as-kernel-probe.sh`. |
| C2 | **AS-2a** — AIC decode | **STARTED 2026-08-14**: `src/platform-select/` does exact `compatible`-string matching and **fails closed** — no prefix, no case fold, no fallback to the newest known revision, because an unrecognised DART driven by the wrong PTE format programs *something*. The AIC table is **empty**, which is the honest state: it grows by adding a spec, not a guess. The event-word decoder needs that spec. | Timer FIQ and IPI delivery |
