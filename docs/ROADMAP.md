...
- **AS-1** **SLICED (2026-08-10).** AS-1 as written below is a dozen independent subsystems, and one spec across all of them would be written entirely against unverified assumptions. It is now delivered in slices, each with its own design doc. **AS-1a is COMPLETE to the hardware gate (`e90ea1e`)**; the rest are unstarted.
  ...
  - **AS-1b — the aarch64 core. Four of nine items done and verified on the machine, 2026-08-16.**
    ...
    | EL2→EL1, SVC entry | **blocked under m1n1, and now understood why.** `HCR_EL2 = 0x32488000038`: `E2H=1` and **`TGE=1`**, so the machine runs in **VHE** with general exceptions trapped to EL2. The EL1-named registers are aliases of the EL2 ones — `TCR_EL1` and `TCR_EL2` read identical (`0x37510b510`), as do `TTBR0_EL1`/`TTBR0_EL2` — and `TGE` means EL1 is not somewhere code can simply be dropped into. Reaching a genuine EL1 means clearing `TGE`, deciding about `E2H`, and building a real EL1 regime: a wholesale reconfiguration that m1n1 depends on not happening. **It belongs in the boot path from iBoot, not in a probe.** The read-only pre-check (`HCR_EL2`, plus `AT s1e1r` for whether a landing pad translates) is in `kernel_probe` and refused correctly |
    ...
    | Timer *interrupt delivery* | **attempted 2026-08-16 and it did not work.** The reasoning that it needs no AIC still looks right — the generic timer is an FIQ source on this platform (m1n1 clears `CNTP_CTL_EL0`/`CNTV_CTL_EL0` "to clear FIQ sources"), `TGE=1` routes physical FIQ to EL2, and `DAIFClr #1` clears the F bit. The interrupt was nonetheless never observed, and the attempt **corrupted its neighbours**: the report buffer came back holding a frequency and two m1n1 addresses in slots this code never writes. Backed out of the probe, kept in `timer::wait_for_interrupt` marked unverified, with the leading candidate recorded — an asynchronous handler runs on whatever stack the interrupted context was using, which is not a stack this code chose |
