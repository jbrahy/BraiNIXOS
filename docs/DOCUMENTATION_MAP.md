...
| [`architecture/BSP-v2-serving-protocol.md`](architecture/BSP-v2-serving-protocol.md) | CURRENT | **The** inbound serving protocol: pre-shared-key handshake, HKDF-SHA256 key schedule, ChaCha20-Poly1305 record layer, the HKDF ratchet, and **both session types** — client (`CapServe`) and admin (`CapAdmin`). ~~Nothing in it is implemented yet; it states so itself.~~ **Partly implemented as of 2026-08-09**: the wire decoder (`src/bsp/`, P2-T3) and the transport cryptography (`src/transport-crypto/`, P2-T2) have landed with Kani harnesses and fuzz corpora. Still unbuilt: `servd`, the admin verb dispatch, the session manager, and the runtime key store — see the spec's own §14 for the authoritative gap list. Delivered by the re-opened P2-T1. |
...
| [`platform-specs/apple-device-tree-format.md`](platform-specs/apple-device-tree-format.md) | CURRENT | The ADT binary format, delivered by AS-0-T1 — the **first** fact table in this directory, and the source `src/adt/` was implemented against under the two-role rule. |
| `platform-specs/*` (further fact tables) | — | ~~None exist yet. The first land with the AS-4 driver chain and AS-5.~~ — **superseded:** the ADT format landed with AS-0-T1, ahead of AS-4. The remaining tables (AIC, DART, RTKit, ANS2, AGX) land with the AS-2 through AS-5 chain. Each is register offsets, struct layouts, sequence diagrams, and state machines — never prose rationale and never code. |
