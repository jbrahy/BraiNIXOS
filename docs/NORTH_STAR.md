...
| Cluster-level domain isolation in the scheduler | AT RISK | Better core utilisation. **Measured 2026-08-17, third revision: under the row-split decomposition decode will actually use, ~4 cores reach ~113 GB/s and a fifth and sixth buy nothing.** So roughly 2 P cores are spare and isolation costing one is close to free. Two earlier figures for this row -- an estimate of 2-4, then a measurement of ~6 from independent matmuls -- are superseded: independent matmuls are a different access pattern and needed more cores to reach the same bandwidth, which is itself an argument for row-splitting | Two domains share an L2. Cross-domain cache side channel — the thing SCHED-03 exists to prevent, re-aimed at clusters. **Design pending 2026-08-17.** |
| The auditor (INV-AUDIT) | DEFAULT | Frees a core and its cache footprint. **Measured: inference saturates at ~4 cores, so ~2 P cores are spare and the auditor can have one at no throughput cost** | Loss of visibility only; it holds no privilege by construction, so removing it costs detection, not containment. **No longer a cut worth making: it costs a core that inference cannot use anyway.** |
